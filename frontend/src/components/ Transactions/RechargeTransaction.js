...
const RechargeTransaction = () => {
  ...
  const fetchTransactions = useCallback(
    async (page) => {
      ...
      try {
        const response = await axios.get(`http://localhost:4321/api/transactions/dayrecharge`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:4321/api/transactions/recharge`, {
        branchId,
        user: userId,
        type,
        description,
