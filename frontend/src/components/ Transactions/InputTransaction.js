...
const InputTransaction = () => {
  ...
  const fetchTransactions = useCallback(
    async (page) => {
      ...
      try {
        const response = await axios.get(`http://localhost:4321/api/transactions/dayinput`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://localhost:4321/api/transactions/input`, {
        branchId,
        user: userId,
        type,
        description,
